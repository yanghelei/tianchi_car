...
from algo_ts.data import Batch, ReplayBuffer, to_numpy, to_torch_as
from algo_ts.utils import MultipleLRSchedulers
...
class BasePolicy(ABC, nn.Module):
    ...
    :class:`~algo_torch.policy.BasePolicy`.

    A policy class typically has the following parts:

    * :meth:`~algo_torch.policy.BasePolicy.__init__`: initialize the policy, including \
        coping the target network and so on;
    * :meth:`~algo_torch.policy.BasePolicy.forward`: compute action with given \
        observation;
    * :meth:`~algo_torch.policy.BasePolicy.process_fn`: pre-process data from the \
        replay buffer (this function can interact with replay buffer);
    * :meth:`~algo_torch.policy.BasePolicy.learn`: update policy with a given batch of \
        data.
    * :meth:`~algo_torch.policy.BasePolicy.post_process_fn`: update the replay buffer \
        from the learning process (e.g., prioritized replay buffer needs to update \
        the weight);
    * :meth:`~algo_torch.policy.BasePolicy.update`: the main interface for training, \
    ...
    Since :class:`~algo_torch.policy.BasePolicy` inherits ``torch.nn.Module``, you can
    use :class:`~algo_torch.policy.BasePolicy` almost the same as ``torch.nn.Module``,
    ...
    def forward(
        ...
        **kwargs: Any,
        ...
        :return: A :class:`~algo_torch.data.Batch` which MUST have the following keys:
    ...
    def map_action(self, act: Union[Batch, np.ndarray]) -> Union[Batch, np.ndarray]:
        ...
        This function is called in :meth:`~algo_torch.data.Collector.collect` and only
    ...
    def map_action_inverse(
        ...
        """Inverse operation to :meth:`~algo_torch.policy.BasePolicy.map_action`.

        This function is called in :meth:`~algo_torch.data.Collector.collect` for
