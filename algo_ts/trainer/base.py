...
class BaseTrainer(ABC):
    ...
    def gen_doc(learning_type: str) -> str:
        ...
        - info dict: result returned from :func:`~algo_torch.trainer.gather_info`
    ...
    def test_step(self) -> Tuple[Dict[str, Any], bool]:
        ...
        if self.verbose:
            ...
            # print(
            #     # "Epoch #%f: test_reward: %.6f ± %.6f, best_reward: %.6f ± %.6f in #%f" %
            #     (self.epoch, rew, rew_std, self.best_epoch, self.best_reward_std, self.best_reward)
            # )
        if not self.is_run:
            test_stat = {
                "test_reward": rew,
